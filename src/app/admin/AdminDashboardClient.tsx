...
export interface AdminDashboardClientProps {
  ...
  totalAllUsersCount: number;
  usersWithPaidPlansCount: number;
  vendors: number;
  ...
  recentRedeemedCoupons: any[];
  recentActivity: any[];
}

const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884d8', '#ff3b30', '#0a5bff', '#d4af37'];

export default function AdminDashboardClient({
  totalUsersCount,
  totalAllUsersCount,
  usersWithPaidPlansCount,
  vendors,
  usedCouponsCount,
  unusedCouponsCount,
  planUserStats,
  ...
  pendingTasksCount,
  recentRedeemedCoupons,
  recentActivity,
}: AdminDashboardClientProps) {
  const [viewMode, setViewMode] = useState<'DATA' | 'GRAPHICAL'>('DATA');

  const barData = [
    { name: 'Financials', Inflow: totalInflow, PaidOut: totalPaidOut, Balance: totalWalletBalance }
  ];

  return (
    <div>
      <h1 style={{ fontSize: '2rem', fontWeight: 'bold', marginBottom: '1rem' }}>Platform Overview</h1>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
        <button
          ...
          style={{
            padding: '0.5rem 1rem',
            background: viewMode === 'DATA' ? 'var(--accent-blue, #0a5bff)' : '#333',
            color: 'white',
            borderRadius: '4px',
            border: 'none',
            cursor: 'pointer',
        ...
        <button
          ...
          style={{
            padding: '0.5rem 1rem',
            background: viewMode === 'GRAPHICAL' ? 'var(--accent-blue, #0a5bff)' : '#333',
            color: 'white',
            borderRadius: '4px',
            border: 'none',
            cursor: 'pointer',
      ...
      {viewMode === 'GRAPHICAL' ? (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem' }}>
          <div style={{ flex: 1, minWidth: '300px', background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '12px' }}>
            <h3 style={{ marginBottom: '1.5rem', color: 'white' }}>User Membership Distribution</h3>
          ...
          <div style={{ flex: 1, minWidth: '300px', background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '12px' }}>
            <h3 style={{ marginBottom: '1.5rem', color: 'white' }}>Financial Overview</h3>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={barData}>
                ...
                <Legend />
                <Bar dataKey="Inflow" fill="#40c76f" name="Total Inflow" />
                <Bar dataKey="PaidOut" fill="#ff3b30" name="Total Paid Out" />
                <Bar dataKey="Balance" fill="#0a5bff" name="Total Wallet Balance" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
